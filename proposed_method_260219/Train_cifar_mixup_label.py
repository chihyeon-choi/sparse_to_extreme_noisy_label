...
model_save_loc = 'checkpoint_noisy_label_mixup_labeled/{}_{}_{}_{}'.format(args.dataset, args.noise_mode, args.r, args.seed)
...
def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader):
    ...
    for batch_idx, (inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x, w_x) in enumerate(labeled_trainloader):      
        ...
        _, logits = net(mixed_input_labeled)
        logits_x = logits
        
        ## Combined Loss
        Lx, lamb = criterion(logits_x, mixed_target_labeled, epoch+batch_idx/num_iter, warm_up)
...
class SemiLoss(object):
    def __call__(self, outputs_x, targets_x, epoch, warm_up):
        Lx = -torch.mean(torch.sum(F.log_softmax(outputs_x, dim=1) * targets_x, dim=1))
        return Lx, linear_rampup(epoch,warm_up)
...
loader = dataloader.cifar_dataloader(args.dataset, r=args.r, noise_mode=args.noise_mode,batch_size=args.batch_size,num_workers=4,\
    root_dir=args.data_path,log=stats_log, seed=args.seed, noise_file='noise_file/%s_%s_%.1f_%d.npz'%(args.dataset, args.noise_mode, args.r, args.seed), root_dir_save='./clean_index_mixup_label')
